    ...
    public class PlayerModelSwitcher : MonoBehaviour
    {
        private void Start()
        {
            try
            {
                ...
                interactionTrigger.onInteract.AddListener(SwitchModel);
            }
            catch (Exception e)
            {
                String errorMessage = "Failed to set up the model switcher on the suits rack!";

                if (Plugin.isTerminalApiLoaded)
                {
                    Plugin.logger.LogWarning(errorMessage + " Use the terminal command to switch models instead.");
                }
                else
                {
